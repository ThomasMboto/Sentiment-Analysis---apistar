...
Thanks to George Berry, the time complexity has been redduced from something like O(N^4) to O(N).
"""

#The packages that will be needed
...
# Got these from http://en.wiktionary.org/wiki/Category:English_degree_adverbs
...
#Class that contains all the fucntions needed to analyze words and sennces
...
#This Block acts as the main - or entry point - of the script
if __name__ == '__main__':

    #Make csv object from 'MessagesToAnalyze.csv' file
    df = pd.read_csv("MessagesToAnalyze.csv")
    #Get column 'SenderMessage' from csv file and save all it's data in a array of string object
    messages = df.SenderMessage

    print("----------------------------------------------------")

    #Declaring a 'SentimentIntensityAnalyzer' object from the class defined above - line 189
    analyzer = SentimentIntensityAnalyzer()
    
    #nltk (Natural Language ToolKit) is leading platform for building Python programs to work with human language data. Learn more @ http://www.nltk.org/
    #tokenize is mainly used to split sentences into arrays of single words - which will then being easily analyzed
    from nltk import tokenize

    import csv

    with open('analysisResults.csv', 'w', newline='') as csvfile:
         #'analysisResults.csv' is the file I'll be writing to. comma will be the delimeter. 'SenderMessage' and 'Polarity Score' will be the column headers
        spamwriter = csv.writer(csvfile, delimiter=',', quoting=csv.QUOTE_MINIMAL)
        spamwriter.writerow(['SenderMessage', 'Polarity Score'])

        for message in messages:

            sentence_list = tokenize.sent_tokenize(str(message))
            paragraphSentiments = 0.0
            for sentence in sentence_list:
                vs = analyzer.polarity_scores(sentence)
                #vs will have the value "{'neg': 0.0, 'neu': 0.254, 'pos': 0.746, 'compound': 0.8316}" - I'll only increment the compound
                paragraphSentiments += vs["compound"]
            #prints to console
            print(str(round(paragraphSentiments/len(sentence_list), 4)))
            #writes the sender message and the compounded polarity score to 'analysisResults.csv'
            spamwriter.writerow([str(message).encode('utf-8'), str(round(paragraphSentiments/len(sentence_list), 4))])
